...
class NarinWikiLib {
	 ...
	 * 최근 업데이트 된 문서 목록
	 */
	public function recentUpdate($wiki_path, $bo_table, $count=5, $subject_len=40) {
		...
		$sql = "SELECT ht.id, ht.wr_id, ht.editor_mb_id, mt.mb_name, mt.mb_nick, ht.reg_date, wt.wr_subject AS docname, nt.ns 
						...
						JOIN ( SELECT MAX(id) AS id FROM {$this->wiki[history_table]} GROUP BY wr_id ORDER BY id DESC LIMIT $count ) AS ht2 ON ht.id = ht2.id
						JOIN {$this->wiki[write_table]} AS wt ON ht.wr_id = wt.wr_id 
						JOIN {$this->wiki[nsboard_table]} AS nt ON nt.wr_id = wt.wr_id AND nt.bo_table = '$bo_table' 
						JOIN {$this->g4[member_table]} AS mt ON wt.mb_id = mt.mb_id";
		
		$res = sql_query($sql);
		$list = array();
		while($row = sql_fetch_array($res)) {
			$href = $wiki_path."/narin.php?doc=".urlencode($this->doc($row[ns], $row[docname]));
