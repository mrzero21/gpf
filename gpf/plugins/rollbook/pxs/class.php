...
$teachers = sql_list("SELECT * FROM {$rb['teacher_table']} WHERE class_id = $rb_class ORDER BY name");

// load student information and sort it
$tmp = sql_list("SELECT * FROM {$rb['student_table']} WHERE class_id = $rb_class ORDER BY name");
...
$dates = sql_list("SELECT date FROM {$rb['rollbook_table']} WHERE class_id = $rb_class ORDER BY date");

$result = sql_query("SELECT rb.date, ct.student_id, ct.stat FROM {$rb['rollbook_table']} AS rb LEFT JOIN {$rb['check_table']} AS ct ON ct.rollbook_id = rb.id LEFT JOIN {$rb['student_table']} AS st ON st.id = ct.student_id WHERE rb.class_id = $rb_class ");									 
...
include_once $rb['skin_path']."/class.skin.php";
